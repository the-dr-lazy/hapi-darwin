 ...
 * @returns {VersionDetails|VersionDetails[]}
 */
const fileHandler = async (file, options) => {
    ...
    const promises = versions.map(
        ({ width, height, suffix = `-${width}x${height}` }) => (
 ...
 * @param {Version[]} options.versions
 * @param {boolean} options.addOriginal
 * @returns {UploadedVersion|(UploadedVersion|UploadedVersion[])[]}
 */
module.exports = async (files, options) => {
    ...
    return options.maxFiles > 1 ? uploads : uploads[0];
};
